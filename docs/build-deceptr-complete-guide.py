...
def add_static_toc(doc):
    ...
    items = [
        "1. Objectif, outils a installer et ordre de construction",
        "2. Erreurs et problemes connus",
        "3. Architecture globale du projet",
        "4. Prerequis et preparation de la machine",
        "5. Construire DECEPTR de zero",
        "6. Installation manuelle depuis le dossier final",
        "7. Configuration manuelle des composants",
        "8. Demarrage manuel de toute l'architecture",
        "9. Configuration manuelle de Kibana et des dashboards",
        "10. Tests utilisateur et validation bout-en-bout",
        "11. Scripts d'automatisation du projet",
        "12. Exploitation SOC et rapport final",
        "13. Maintenance, sauvegarde et nettoyage Docker",
...
def section_overview_tools(doc):
    doc.add_heading("1. Objectif, outils a installer et ordre de construction", level=1)
    add_para(
        doc,
        "Cette premiere section explique ce qu'il faut installer, pourquoi chaque outil existe dans DECEPTR, et dans quel ordre construire le projet. L'objectif est de comprendre le role de chaque brique avant de lancer les commandes.",
    )
    add_callout(
        doc,
        "Objectif global de DECEPTR",
        "Creer un piege credible pour attaquants externes et utilisateurs internes malveillants: faux services, faux portail intranet, fichiers leurres, honeytokens, collecte des actions, enrichissement, scoring, alertes et dashboards SOC.",
        fill=LIGHT_BLUE,
    )
    doc.add_heading("1.1 Ce qu'il faut installer sur la machine", level=2)
    add_table(
        doc,
        ["Outil a installer", "Role dans le projet", "Pourquoi il est necessaire"],
        [
            ["Windows 10/11 + WSL2", "Base d'execution Linux pour Docker Desktop", "Permet de faire tourner les conteneurs Linux comme Elasticsearch, Logstash, Cowrie, Redis et MySQL."],
            ["Docker Desktop", "Moteur de conteneurisation", "Lance tous les services DECEPTR de maniere reproductible sans installer chaque serveur manuellement dans Windows."],
            ["Docker Compose", "Orchestrateur local", "Demarre l'architecture complete avec un seul fichier: reseaux, volumes, ports, dependances et variables."],
            ["PowerShell", "Automatisation Windows", "Execute `start.ps1`, `stop.ps1`, les tests E2E et les commandes de verification."],
            ["Git", "Gestion de version et transport du projet", "Permet de cloner, versionner et livrer le dossier projet proprement."],
            ["Python 3", "Scripts de documentation et outils hors conteneur", "Sert a generer DOCX/PDF, tests locaux et scripts utilitaires. Le pipeline runtime utilise Python dans Docker."],
            ["Node.js", "Generation HTML des schemas", "Utilise par `build-pdfs.js` pour generer les pages HTML des architectures."],
            ["GeoLite2-City.mmdb", "Base locale de geolocalisation IP", "Permet d'enrichir les evenements avec pays, ville et contexte reseau sans dependance externe obligatoire."],
            ["Navigateur web", "Acces aux interfaces", "Permet d'ouvrir Dashboard, Kibana, API Docs, MinIO et web honeypot."],
        ],
        widths=[1.55, 2.05, 2.9],
    )
    doc.add_heading("1.2 Role de chaque composant DECEPTR", level=2)
    add_table(
        doc,
        ["Etape", "Composant", "Travail effectue", "Objectif securite"],
        [
            ["1", "T-Pot Cowrie", "Expose SSH/Telnet leurres, capture login, commandes et sessions", "Attirer l'attaquant vers un systeme controle au lieu du vrai SI."],
            ["2", "Web honeypot", "Faux portail intranet avec faux documents et login capture", "Pieger les tentatives d'acces applicatif interne et voler le temps de l'attaquant."],
            ["3", "Dionaea", "Expose FTP, HTTP, SMB et MySQL decoy", "Detecter scans, malware, enumeration de services et attaques automatises."],
            ["4", "Filebeat", "Lit les logs Cowrie et les envoie a Logstash", "Transporter les traces sans perdre les evenements."],
            ["5", "TLS 1.3", "Chiffre Filebeat vers Logstash sur 5044", "Eviter l'interception ou la falsification des logs en transit."],
            ["6", "Logstash", "Parse, structure et route les evenements", "Transformer les logs bruts en donnees exploitables."],
            ["7", "Redis", "File d'attente `deceptr:events`", "Absorber les pics de logs et decoupler ingestion/analyse."],
            ["8", "Pipeline Python", "Collecte, normalise, enrichit, correle, score et detecte", "Transformer une trace technique en alerte SOC qualifiee."],
            ["9", "Threat Intel", "GeoLite2, VirusTotal, AbuseIPDB, Feodo, MITRE ATT&CK", "Ajouter contexte, reputation, geolocalisation, C2 et techniques adverses."],
            ["10", "Elasticsearch", "Stocke logs bruts et evenements enrichis", "Recherche rapide, investigation et Kibana."],
            ["11", "MySQL", "Stocke alertes, IoC, attackers, campaigns, honeytokens, users", "Conserver les donnees metier du SOC et du dashboard."],
            ["12", "MinIO", "Stockage objet pour rapports, samples, backups et attachments", "Garder les preuves et objets lies aux incidents."],
            ["13", "FastAPI", "Expose API REST securisee par JWT", "Permettre au dashboard de lire alertes, stats, IoC et rapports."],
            ["14", "Dashboard DECEPTR", "Interface SOC simple", "Voir rapidement l'etat, les alertes, IoC, campagnes et rapports."],
            ["15", "Kibana", "Exploration ELK avancee", "Investiguer les logs bruts et creer des vues analytiques."],
            ["16", "ElastAlert 2", "Regles et notifications", "Alerter sur les evenements critiques sans attendre une consultation manuelle."],
        ],
        widths=[0.45, 1.35, 2.25, 2.45],
    )
    doc.add_heading("1.3 Ordre logique de construction", level=2)
    numbered(
        doc,
        [
            "Preparer Windows, WSL2 et Docker Desktop.",
            "Creer le dossier final `D:\\assir\\Ismagi\\PFA\\DECEPTR-FINAL`.",
            "Creer les reseaux Docker, volumes, `.env` et secrets.",
            "Installer ou lancer le runtime T-Pot/Cowrie limite au besoin du projet.",
            "Configurer Filebeat et les certificats TLS vers Logstash.",
            "Demarrer Logstash, Redis, Elasticsearch, MySQL et MinIO.",
            "Construire le pipeline Python et l'API FastAPI.",
            "Ajouter le web honeypot, Dionaea et les honeytokens.",
            "Configurer Kibana, Dashboard DECEPTR et ElastAlert.",
            "Ajouter GeoLite2 et les cles API Threat Intel si disponibles.",
            "Lancer le smoke test E2E puis corriger les erreurs.",
            "Documenter le resultat et sauvegarder les preuves.",
        ],
    )
    doc.add_heading("1.4 Ce que DECEPTR doit prouver pendant la demonstration", level=2)
    add_table(
        doc,
        ["Preuve attendue", "Comment la montrer"],
        [
            ["Un attaquant peut interagir avec les leurres", "Connexion SSH/Telnet Cowrie, login web honeypot ou acces Dionaea."],
            ["Les logs arrivent securises", "`filebeat test output` affiche TLS 1.3 OK."],
            ["Le pipeline comprend l'attaque", "Evenement enrichi dans `deceptr-events-*` avec MITRE, score et severite."],
            ["Le SOC voit l'incident", "Dashboard DECEPTR affiche alertes, stats, IoC et campagnes."],
            ["Le canary detecte une fuite", "`/api/canary/trigger/{token}` genere une alerte CRITICAL T1039."],
            ["Les preuves sont conservees", "Elasticsearch, MySQL et MinIO contiennent les donnees."],
        ],
        widths=[2.1, 4.4],
    )
...
def section_errors(doc):
    doc.add_heading("2. Erreurs et problemes connus - Realite du terrain", level=1)
    add_para(
        doc,
...
def section_architecture(doc):
    doc.add_heading("3. Architecture globale du projet", level=1)
...
""",
    ...
    doc.add_heading("3.1 Dossier final", level=2)
    add_code(
        doc,
        r"""
...
def section_prereq(doc):
    doc.add_heading("4. Prerequis et preparation de la machine", level=1)
    add_table(
        doc,
    ...
    doc.add_heading("4.1 Verifications Windows", level=2)
    add_code(
        doc,
        r"""
docker version
...
def section_install(doc):
    doc.add_heading("6. Installation manuelle depuis le dossier final", level=1)
    doc.add_heading("6.1 Recuperer le dossier final", level=2)
...
""",
    ...
    doc.add_heading("6.2 Installer Docker Desktop", level=2)
    numbered(
        doc,
        [
    ...
    doc.add_heading("6.3 Charger les images manuellement", level=2)
...
""",
    ...
    doc.add_heading("6.4 Construire les images Python", level=2)
    add_code(
        doc,
        r"""
...
def section_build_zero(doc):
    doc.add_heading("5. Construire DECEPTR de zero", level=1)
    add_para(
        doc,
    ...
    doc.add_heading("5.1 Creer l'arborescence", level=2)
    add_code(
        doc,
        r"""
...
""",
    ...
    doc.add_heading("5.2 Creer le fichier .env", level=2)
...
""",
    ...
    doc.add_heading("5.3 Generer les certificats TLS", level=2)
    add_code(
        doc,
        r"""
...
""",
    ...
    doc.add_heading("5.4 Creer docker-compose.yml DECEPTR", level=2)
...
""",
    ...
    doc.add_heading("5.5 Creer le runtime Cowrie T-Pot", level=2)
...
""",
    ...
    doc.add_heading("5.6 Creer Filebeat T-Pot", level=2)
...
""",
    ...
    doc.add_heading("5.7 Creer Logstash", level=2)
...
""",
    ...
    doc.add_heading("5.8 Creer MySQL schema.sql", level=2)
...
""",
    ...
    doc.add_heading("5.9 Creer le pipeline Python", level=2)
...
""",
    ...
    doc.add_heading("5.10 Creer dashboard et API", level=2)
    numbered(
        doc,
        [
    ...
    doc.add_heading("5.11 Creer start.ps1 et stop.ps1", level=2)
    add_code(
        doc,
        r"""
# start.ps1
...
""",
    ...
    doc.add_heading("5.12 Validation d'un build zero", level=2)
    add_code(
        doc,
        r"""
cd D:\assir\Ismagi\PFA\DECEPTR-FROM-ZERO
...
def section_config(doc):
    doc.add_heading("7. Configuration manuelle des composants", level=1)
    doc.add_heading("7.1 Fichier .env principal", level=2)
    ...
    doc.add_heading("7.2 Certificats TLS Filebeat -> Logstash", level=2)
...
""",
    ...
    doc.add_heading("7.3 T-Pot Cowrie et Filebeat", level=2)
    ...
    doc.add_heading("7.4 Logstash", level=2)
...
""",
    ...
    doc.add_heading("7.5 Pipeline Python", level=2)
    add_table(
        doc,
        ["Fichier", "Role"],
        [
    ...
    doc.add_heading("7.6 Comptes par defaut", level=2)
    add_table(
        doc,
...
def section_start(doc):
    doc.add_heading("8. Demarrage manuel de toute l'architecture", level=1)
    doc.add_heading("8.1 Methode recommandee", level=2)
    add_code(
        doc,
        r"""
...
""",
    ...
    doc.add_heading("8.2 Methode totalement manuelle", level=2)
...
""",
    ...
    doc.add_heading("8.3 Verifier l'etat", level=2)
    add_code(
        doc,
        r"""
docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
...
def section_kibana(doc):
    doc.add_heading("9. Configuration manuelle de Kibana et des dashboards", level=1)
    numbered(
        doc,
        [
...
def section_tests(doc):
    doc.add_heading("10. Tests utilisateur et validation bout-en-bout", level=1)
    doc.add_heading("10.1 Test interactif SSH/Telnet", level=2)
    add_code(
        doc,
        r"""
...
""",
    ...
    doc.add_heading("10.2 Test E2E automatise", level=2)
    add_code(
        doc,
        r"""
...
""",
    ...
    doc.add_heading("10.3 Verification API", level=2)
    add_code(
        doc,
        r"""
...
def section_scripts(doc):
    doc.add_heading("11. Scripts d'automatisation du projet", level=1)
    add_table(
        doc,
    ...
    doc.add_heading("11.1 Script start.ps1", level=2)
...
""",
    ...
    doc.add_heading("11.2 Script stop.ps1", level=2)
    add_code(
        doc,
        r"""
cd D:\assir\Ismagi\PFA\DECEPTR-FINAL
...
def section_ops(doc):
    doc.add_heading("12. Exploitation SOC et rapport final", level=1)
    ...
    doc.add_heading("12.1 Structure du rapport final", level=2)
    add_table(
        doc,
        ["Section", "Contenu"],
        [
...
def section_maintenance(doc):
    doc.add_heading("13. Maintenance, sauvegarde et nettoyage Docker", level=1)
    doc.add_heading("13.1 Sauvegarder les donnees", level=2)
    add_code(
        doc,
        r"""
...
""",
    ...
    doc.add_heading("13.2 Nettoyer uniquement DECEPTR", level=2)
...
""",
    ...
    doc.add_heading("13.3 Logs utiles", level=2)
    add_code(
        doc,
        r"""
...
def section_checklist(doc):
    doc.add_heading("14. Checklist complete", level=1)
    add_table(
        doc,
